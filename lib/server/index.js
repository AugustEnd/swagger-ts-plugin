...
function startCreate({ outputPath, serverList, appUrl, }) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            const values = yield Promise.all(serviceArr.map((item) => request_1.getSimpleServiceData({
                serviceName: item.serviceName,
                serviceUrl: item.serviceUrl,
            })));
            const paths = yield Promise.all(values
                .filter((el) => el)
                .map((el) => __awaiter(this, void 0, void 0, function* () {
                try {
                    yield index_1.completeInterfaceAll(el, {
                        name: el.serviceName,
                        path: path.resolve(outputPath || +__dirname, "./swagger2ts"),
                    });
                    yield index_2.completePathAll(el.paths, {
                        name: el.serviceName,
                        path: path.resolve(outputPath || +__dirname, "./swagger2ts"),
                    });
