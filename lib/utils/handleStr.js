...
const completeInterfaceAll = (interfaceObj, options) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        console.log("文件地址", `${newOptions.path}/${newOptions.fileName.replace(/\[\S*\]/g, newOptions.name)}`);
        fs.writeFile(`${newOptions.path}/${newOptions.fileName.replace(/\[\S*\]/g, newOptions.name)}`, str, () => { });
    }
    catch (error) { }
});
